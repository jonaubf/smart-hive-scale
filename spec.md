...
- [ ] Copy `.env.example` → `.env` with static IP and MQTT credentials
- [ ] Generate CA/server certs and configure Mosquitto TLS (Step 9)
- [ ] Router port forward **8883** only
- [ ] Copy `ca.crt` → `certs/ca.pem` for firmware builds
- [ ] Create Mosquitto user for `hive-01`
- [ ] Configure Home Assistant MQTT sensors and dashboard (Step 10) — see [`doc/user-guide.md`](doc/user-guide.md)
- [ ] Battery capacity and expected runtime measurement
